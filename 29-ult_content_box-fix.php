...
<?php
function remove_ult_content_box_shortcodes() {
    ...
    foreach ($post_types as $post_type) {
        ...
        $args = [
            ...
            'offset'         => (int) get_option('remove_ult_content_box_offset_' . $post_type, 0),
        ];
        ...
        while ($query->have_posts()) {
            ...
            // Remove all occurrences of [ult_content_box] with attributes and variations
        ...
        $current_offset = (int) get_option('remove_ult_content_box_offset_' . $post_type, 0);
        $new_offset = $current_offset + $processed_posts;

        if ($new_offset >= $query->found_posts) {
            update_option('remove_ult_content_box_done_' . $post_type, true);
            delete_option('remove_ult_content_box_offset_' . $post_type);
        } else {
            update_option('remove_ult_content_box_offset_' . $post_type, $new_offset);
        }
...
add_action('init', function () {
    ...
    $post_types = get_post_types(['public' => true], 'names');
    foreach ($post_types as $post_type) {
        if (!get_option('remove_ult_content_box_done_' . $post_type)) {
            $done_for_all_post_types = false;
            remove_ult_content_box_shortcodes();
            break; // Process one post type at a time per request
        }
    }

    if ($done_for_all_post_types) {
        echo '<script>console.log("All post types processed successfully, [ult_content_box] and variations removed.");</script>';
    }
});
