...
<?php
function remove_vc_empty_space_shortcodes() {
    ...
    foreach ($post_types as $post_type) {
        ...
        $args = [
            ...
            'offset'         => (int) get_option('remove_vc_empty_space_offset_' . $post_type, 0),
        ];
        ...
        while ($query->have_posts()) {
            ...
            // Remove all occurrences of [vc_empty_space] with attributes and variations
            $updated_content = preg_replace('/\[vc_empty_space[^\]]*\]/', '', $content);
        ...
        $current_offset = (int) get_option('remove_vc_empty_space_offset_' . $post_type, 0);
        $new_offset = $current_offset + $processed_posts;

        if ($new_offset >= $query->found_posts) {
            update_option('remove_vc_empty_space_done_' . $post_type, true);
            delete_option('remove_vc_empty_space_offset_' . $post_type);
        } else {
            update_option('remove_vc_empty_space_offset_' . $post_type, $new_offset);
        }
...
add_action('init', function () {
    ...
    $post_types = get_post_types(['public' => true], 'names');
    foreach ($post_types as $post_type) {
        if (!get_option('remove_vc_empty_space_done_' . $post_type)) {
            $done_for_all_post_types = false;
            remove_vc_empty_space_shortcodes();
            break; // Process one post type at a time per request
        }
    }

    if ($done_for_all_post_types) {
        echo '<script>console.log("All post types processed successfully, [vc_empty_space] and variations removed.");</script>';
    }
});
