...
<?php
function remove_vc_row_inner_closing_shortcodes() {
    ...
    foreach ($post_types as $post_type) {
        ...
        $args = [
            ...
            'offset'         => (int) get_option('remove_vc_row_inner_closing_offset_' . $post_type, 0),
        ];
        ...
        $current_offset = (int) get_option('remove_vc_row_inner_closing_offset_' . $post_type, 0);
        $new_offset = $current_offset + $processed_posts;

        if ($new_offset >= $query->found_posts) {
            update_option('remove_vc_row_inner_closing_done_' . $post_type, true);
            delete_option('remove_vc_row_inner_closing_offset_' . $post_type);
        } else {
            update_option('remove_vc_row_inner_closing_offset_' . $post_type, $new_offset);
        }
